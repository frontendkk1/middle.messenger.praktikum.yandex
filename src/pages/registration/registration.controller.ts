...
export class RegistrationController {
    ...
    public async signup(data: RegistrationFormModel) {
        try {
            ...
            const validatePassword = validate(ValidationNames.PASSWORD, data.password);
            ...
            if (!validatePassword.isValid) notValidMessages.push(validatePassword.message);
            ...
            if (profileResponse.status !== 200) {
                throw new Error(profileResponse.response?.reason || 'Что-то пошло не так');
            }

            this.router.go('/chats');
        } catch (e) {
            console.log(e)
            store.set('signupReq', { isLoading: false, errorMessage: e.message })
        }
    }
}
